...
const OldFaIconComponent = require('./build-time-components/old-fa-icon');
...
class AstTransform {
  ...
  _applyTransform(node) {
    if (node.path.original === 'old-fa-icon') {
      return new OldFaIconComponent(node, { transform: this }).toElement()
