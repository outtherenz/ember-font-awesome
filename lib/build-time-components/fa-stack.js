    ...
    this.contentVisitor = {
      MustacheStatement: (node) => {
        ...
        if (node.path.original === `${componentAlias}.stack-1x`) {
          return (new OldFaIconComponent(node, { stack: '1', transform: opts.transform })).toElement();
        } else if (node.path.original === `${componentAlias}.stack-2x`) {
          return (new OldFaIconComponent(node, { stack: '2', transform: opts.transform })).toElement();
        }
      }
    }
  }
